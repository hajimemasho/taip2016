	...
	public Polynomial computeComponent(int i, int j) {
		...
		if (i == j) {
			...
			if (!nominator.equals(BigInteger.ZERO)) {
				Polynomial p1 = computeComponent(i, j - 1);
				Polynomial member1 = p1.multiplyByConstant(this.getPoints().get(j).getX());
				Polynomial member2 = p1.multiplyByX();
				
				Polynomial p2 = computeComponent(i + 1, j);
				Polynomial member3 = p2.multiplyByX();
				Polynomial member4 = p2.multiplyByConstant(this.getPoints().get(i).getX());
